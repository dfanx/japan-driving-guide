...
for (const localeCase of [
  ...
  test(`@f021 ${localeCase.locale} traceability route exposes full evidence`, async ({ page }) => {
    ...
    await expect(page.locator(".source-entry")).toHaveCount(39);
    await expect(page.locator(".classification-card")).toHaveCount(3);
    await expect(page.locator(".classification-card li")).toHaveCount(56);
    ...
    await expect(page.locator('time[datetime="2026-08-14"]')).toHaveCount(6);
