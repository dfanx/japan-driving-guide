...
test("@f014 @f030 @f031 zh-TW completes all 25 questions and points to weak lessons", async ({ page }) => {
  ...
  await expect(result).toContainText("0 / 25");
...
test("@f014 @f030 @f031 en completes all 25 questions without claiming certification", async ({ page }) => {
  await page.goto("/en/review/");
  await completeReview(page, true);

  const result = page.locator("[data-review-result]");
  await expect(result).toBeVisible();
  await expect(result).toContainText("25 / 25");
  ...
  await expect(page.getByRole("button", { name: "Try all 25 again" })).toBeVisible();
});
...
test("@f014 @f030 360px final result stays readable", async ({ page }) => {
  ...
  expect((await page.getByRole("button", { name: "重新做 25 題" }).boundingBox())?.height).toBeGreaterThanOrEqual(44);
