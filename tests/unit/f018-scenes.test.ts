...
describe("F018 semantic Scene inventory", () => {
  ...
  it("encodes the F032 safety semantics in deterministic SVG data", () => {
    ...
    const d025 = buildDiagramArtifact(sceneById("D025")).svg;
    expect(d025).toContain('data-guide-strip="crossable-white"');
    expect(d025).toContain('data-entry-prohibited="yellow-bordered"');
    expect(d025).toContain('data-risk="vehicle-in-hatching"');
    expect(d025).toContain('data-turn-lane="right"');
